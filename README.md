...
基于 LSPosed 的通知过滤模块：在 `system_server` 里拦下不想看到的通知，支持关键词 / 正则规则和**完全离线的 AI 骚扰识别**，并且可以在手机上用自己的标注微调模型。
...
**端侧微调**
- 在记录详情里把通知标为「骚扰」或「正常」，App 会立即在本地重新拟合一个叠加在内置模型上的稀疏修正量，并推送给 `system_server` 生效。
